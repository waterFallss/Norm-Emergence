...
def initial_set_run(num_agents:int, num_exp_runs:int, k_last_action_second:int, network_type:str,
    ...
    # Keep track of the latest decisions of the agents
    latest_decisions = [[-1]*matrix_col for _ in range(matrix_row)]    
    ...
    for x in range(num_exp_runs):
        ...
        if x==0:
            for agent in oned_agent_list:
                action_one = agent.id
                agent.first_player_action.append(action_one)
            print_matrix(latest_decisions)
        ...
        for pair in paired_agents:
            ...
            # Keep track of the latest decisions of the agents
            latest_decisions[agent_one.id//matrix_col][agent_one.id%matrix_col] = action_one
            latest_decisions[agent_two.id//matrix_col][agent_two.id%matrix_col] = action_two
...
def run_exp(num_agents:int, num_exp_runs:int, k_last_action_second:int, network_type:str,
    ...
    # Keep track of the latest decisions of the agents
    latest_decisions = [[0]*matrix_col for _ in range(matrix_row)]    
    print("Latest Decisions: ", latest_decisions)
    ...
    for x in range(num_exp_runs):
        ...
        for pair in paired_agents:
            ...
            # Keep track of the latest decisions of the agents
            latest_decisions[agent_one.id//matrix_col][agent_one.id%matrix_col].append(action_one)
            latest_decisions[agent_two.id//matrix_col][agent_two.id%matrix_col].append(action_two)
...
def main():  
    ...
    for option in options:
        ...
        for _ in range(num_experiment):
            _, _, _, _, _, _, _, _, frac_k_last, first_conv = initial_set_run(num_agents,
            ...
            plt.ylim(0, 1)
            plt.title("Frequency of Action Taken " + str(k_last_ep) + " Episodes")
            plt.xlabel("Episode", fontsize=14)
            plt.ylabel("Frequency of Action Taken", fontsize=14)
            plt.show()
            clear_all()
