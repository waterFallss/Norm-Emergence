...
def find_first_convergence(num_agents:int, num_exp_runs:int, k_last_action_second:int, network_type:str,
...
def main():      
    ...
    for num_agent in agent_num:
        ...
        for _ in range(num_experiment):
            episode_conv = find_first_convergence(num_agents=num_agent, num_exp_runs=num_episodes, k_last_action_second=10
            , network_type="grid", exploration_type="epsilon_greedy", matrix_row=agent_row, matrix_col=agent_col
            , num_neighbor=4, x_conv=1)
            first_convs.append(episode_conv)
            clear_all()
    print("Convergence Points: ", first_convs)
    print("Average Convergence Point: ", np.mean(first_convs))
    print("Standard Deviation: ", np.std(first_convs))
    print("Variance: ", np.var(first_convs))
