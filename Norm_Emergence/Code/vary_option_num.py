...
def create_agents_matrix(agent_list, matrix_row=0, matrix_col=0, type="grid") -> list:
    ...
    if type == "scale_free":
        ...
        return agent_list
    print("Invalid Neighborhood Option.")
    sys.exit()
    return []
...
def main():   
    ...
    for option in options:
        ...
        for _ in range(num_experiment):
            _, _, _, _, _, _, _, _, first_player_freq, first_conv = run_exp(num_agents,
            ...
            colors = cm.viridis(np.linspace(0, 1, num_options))
            fig, ax = plt.subplots()
            for i in range(num_options):
                # Studying Convergence
                ax.scatter(x_axis, first_player_freq[i], c=[colors[i]], alpha=1)
                # Studying Blip
                # ax.scatter(range(len(first_player_freq[i][point_one:point_two])), first_player_freq[i][point_one:point_two], c=[colors[i]], alpha=1)
            ...
            ax.set_ylabel("Frequency of Chosen Actions", fontsize=16)
            # Varying Options Number
            # ax.set_xticks(np.arange(0, num_episodes+1, 1000))
            # Studying Blip
            # ax.set_xticks(np.arange(0, len(first_player_freq[0][point_one:point_two]), 10))
            plt.ylim(0,1)
            plt.show()
