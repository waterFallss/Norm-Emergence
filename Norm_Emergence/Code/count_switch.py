...
weight_two = 1 # Observation as Second Player
weight_three = 0 # All Neighbors
...
# num_exp_runs: number of experimental runs
def count_switch(num_agents:int, num_exp_runs:int, k_last_action_second:int, network_type:str,
            exploration_type="epsilon_greedy", matrix_row=0, matrix_col=0, num_neighbor=0, x_conv=10):
    """_summary_
    ...
    Args:
        ...
        matrix_col (int): Indicate the dimensions of the matrix column. Defaults to 0.
        num_neighbor (int):  Defaults to 0.
        x_conv (int): Number of convergences to check for.
    Returns:
        # TO DO: Before pushing to Github
    """
    global temperature, episode_num, action_one_per_episode, num_options
    ...
    # Keep track of the fraction of times each action was taken over the last k (100) episodes
    frac_opt_chosen_k_last = np.zeros((num_options, num_exp_runs), dtype=float)
    option_chosen = [[] for _ in range(num_options)] # Number of times each option was chosen per episode
    ...
    paired_agents, neighbors = matrix_pairing(agent_list, num_neighbor, oned_agent_list, type=network_type)
    # Convergence Counter
    # Counts until there are x_conv convergences in a row
    conv_counter = 0
    # action that was converged into in the previous episode
    prev_converged_action = -1
    # Episode of Convergence
    episode_of_conv = -1
    # Count the number of Convergence Switches that occur
    switch_counter = 0
    # Store the last action that the most recent x_conv convergences converged to
    prev_x_conv_action = -1
    ...
    for x in range(num_exp_runs):
        ...
        # Fraction of time each action was taken over the episode just complete
        passed_threshold = False
        for action in range(len(options)):
            fraction = options[action]/(num_agents*2)
            if fraction >= 0.9:
                if prev_converged_action == action:
                    conv_counter += 1
                else:
                    prev_converged_action = action
                    conv_counter = 1
                passed_threshold = True
        if passed_threshold==False:
            conv_counter = 0
        if conv_counter == x_conv:
            if prev_x_conv_action != -1 and prev_converged_action != prev_x_conv_action:
                switch_counter += 1
            prev_x_conv_action = prev_converged_action
            prev_converged_action = -1
            conv_counter = 0
    ...
    # Fraction of time each action was taken over the last k episodes
    for i in range(num_options):
        frac_opt_chosen_k_last[i] = [x/(num_agents*2) for x in option_chosen[i][-num_exp_runs:]]
    # Agent Actions: Dictionary of agents and the actions they have made
    # Option Chosen: Given index of action number, it will show the list of times that action was chosen per episode
    return switch_counter, frac_opt_chosen_k_last
...
def main():    
    ...
    k_last_action_second = 2
    x_conv = 10 # Number of Episodes in which convergence to a specific action must occur to count as switch
    num_agents = 100
    num_episodes = 5000
    num_rounds = 20
    count = 0
    ...
    for x in range(num_rounds):
        num_switch, frac_option_chosen = count_switch(num_agents=num_agents, num_exp_runs=num_episodes, k_last_action_second=k_last_action_second
            , network_type="grid", exploration_type="epsilon_greedy", matrix_row=10, matrix_col=10
            , num_neighbor=4, x_conv=x_conv)
        num_conv_switch.append(num_switch)
        count += 1
        print("Number of Switches: ", num_switch, "Count: ", count)
        clear_all()
        
        # Graphing to Check Results ##########################################################
        # # Fraction of time each action was taken over the last k episodes
        ...
        # fig, ax = plt.subplots()
        # for i in range(num_options):
        #     ax.scatter(x_axis, frac_option_chosen[i], c=[colors[i]], alpha=1)
    ...
    print("Episodes to Convergence: ", num_conv_switch)
